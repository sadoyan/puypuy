        ...
        try:
            ...
            for key in list(mon_values.keys()):
                if key is 'datanode_dfsused' or key is 'datanode_space_remaining':
                    self.local_vars.append({'name': key.lower(), 'timestamp': self.timestamp, 'value': mon_values[key], 'check_type': check_type, 'reaction': reaction, 'chart_type': 'Rate'})
                else:
                    self.local_vars.append({'name': key.lower(), 'timestamp': self.timestamp, 'value': mon_values[key], 'check_type': check_type, 'reaction': reaction})
    
            data_du_percent = mon_values['datanode_dfsused']*100/(mon_values['datanode_dfsused']+mon_values['datanode_space_remaining'])
            self.local_vars.append({'name': 'datanode_du_percent', 'timestamp': self.timestamp, 'value':data_du_percent, 'check_type': check_type, 'reaction': reaction, 'chart_type': 'Percent'})
