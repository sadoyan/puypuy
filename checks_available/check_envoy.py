...
meters = (
            'envoy_http_downstream_cx_total','downstream_cx_ssl_total','downstream_cx_http1_total', 'envoy_http_downstream_cx_destroy_remote'
            'downstream_cx_websocket_total','downstream_cx_http2_total','downstream_cx_destroy',
            'downstream_cx_destroy_remote','downstream_cx_destroy_remote','downstream_cx_destroy_active_rq',
            ...
            'envoy_http_downstream_cx_http1_total', 'envoy_http_downstream_cx_http2_total', 'envoy_http_downstream_cx_http3_total',
            'envoy_http_downstream_cx_websocket_total', 'envoy_http_downstream_cx_grpc_total'
        )

fortotal = {'envoy_http_downstream_rq_http1_total', 'envoy_http_downstream_rq_http2_total', 'envoy_http_downstream_rq_http3_total',
            'envoy_http_downstream_rq_websocket_total', 'envoy_http_downstream_rq_grpc_total'}
        ...
        try:
            ...
            for c in data:
                if any(x in c for x in meters) and c.startswith('envoy_') and 'admin' not in c and '_bucket' not in c:
                    ...
                    if o[0] in fortotal:
                        total_rps = total_rps + int(o[-1])
                    if o[0] != "envoy_listener_worker_downstream_cx_active":
            ...
            rate = self.rate.record_value_rate('envoy_requests_total', total_rps, self.timestamp)
            self.local_vars.append({'name': 'envoy_requests_rate', 'timestamp': self.timestamp, 'value': rate, 'check_type': check_type})
