        ...
        try:
            ...
            for beans in jolo_mbeans:
                ...
                if beans == 'org.apache.cassandra.metrics:type=Compaction,name=*':
                    ...
                    self.local_vars.append({'name': name.lower(), 'timestamp': self.timestamp, 'value': mon_values, 'check_type': check_type})
                    ...
                    for my_name in jolo_keys:
                        ...
                        if my_name in needed_stats and my_value > 0:
                            name = 'cassa_' + my_name
                            value_rate = self.rate.record_value_rate(name, my_value, self.timestamp)
                            self.local_vars.append({'name': name.lower(), 'timestamp': self.timestamp, 'value': value_rate, 'check_type': check_type, 'chart_type': 'Rate'})
                ...
                if beans == 'org.apache.cassandra.request:type=*':
                    for key in request_keys:
                        ...
                        value_rate = self.rate.record_value_rate(name, value, self.timestamp)
                        self.local_vars.append({'name': name.lower(), 'timestamp': self.timestamp, 'value': value_rate, 'check_type': check_type, 'chart_type': 'Rate'})
            ...
            if CMS is True:
                ...
                for coltype in collector:
                    ...
                    def push_metrics(preffix):
                        self.local_vars.append({'name': 'cassa_' + preffix + '_collection_count', 'timestamp': self.timestamp, 'value': CollectionCount, 'check_type': check_type})
                        CollectionTime_rate = self.rate.record_value_rate('cassa_' + preffix + '_collection_time', CollectionTime, self.timestamp)
                        self.local_vars.append({'name': 'cassa_' + preffix + '_collection_time', 'timestamp': self.timestamp, 'value': CollectionTime_rate, 'check_type': check_type, 'chart_type': 'Rate'})
                        if 'LastGcInfo' in locals():
                            self.local_vars.append({'name': 'cassa_' + preffix + '_lastgcinfo', 'timestamp': self.timestamp, 'value': LastGcInfo, 'check_type': check_type})
                ...
                for k, v in enumerate(gc_g1):
                    ...
                    for ky, vl in enumerate(metr_keys):
                        if ky is 0:
                            ...
                            v = check_null(value)
                            rate_key = vl + type
                            CollectionTime_rate = self.rate.record_value_rate('cassa_' + rate_key, v, self.timestamp)
                            self.local_vars.append({'name': 'cassa_g1' + type + vl, 'timestamp': self.timestamp, 'value': vl, 'check_type': check_type})
                        if ky is 1:
                            value = j['value'][vl]
                            v = check_null(value)
                            self.local_vars.append({'name': 'cassa_g1' + type + vl, 'timestamp': self.timestamp, 'value': v, 'check_type': check_type})
