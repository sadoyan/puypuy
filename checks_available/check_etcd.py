            ...
            for c in data:
                if any(x in c for x in greps) and '#' not in c:
                    ...
                    if c.startswith('etcd_'):
                        self.local_vars.append({'name': o[0], 'timestamp': self.timestamp, 'value': o[1], 'reaction': reaction})
                    else:
                        self.local_vars.append({'name': 'etcd_' + o[0], 'timestamp': self.timestamp, 'value': o[1], 'reaction': reaction})
