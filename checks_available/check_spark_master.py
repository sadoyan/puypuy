            ...
            if CMS is True:
                ...
                for coltype in collector:
                    ...
                    def push_metrics(preffix):
                        CollectionTime_rate = self.rate.record_value_rate('spark_master_' + preffix + '_collectiontime', CollectionTime, self.timestamp)
                        self.local_vars.append({'name': 'spark_master_' + preffix + '_lastgcinfo', 'timestamp': self.timestamp, 'value': LastGcInfo, 'check_type': check_type})
                        self.local_vars.append({'name': 'spark_master_' + preffix + '_collection_count', 'timestamp': self.timestamp, 'value': CollectionCount, 'check_type': check_type})
                        self.local_vars.append({'name': 'spark_master_' + preffix + '_collectiontime', 'timestamp': self.timestamp, 'value': CollectionTime_rate, 'check_type': check_type, 'chart_type': 'Rate'})
                ...
                for k, v in enumerate(gc_g1):
                    ...
                    for ky, vl in enumerate(metr_keys):
                        if ky is 0:
                            ...
                            v = check_null(value)
                            rate_key = vl + type
                            CollectionTime_rate = self.rate.record_value_rate('spark_master_' + rate_key, v, self.timestamp)
                            self.local_vars.append({'name': 'spark_master_g1' + type + vl.lower(), 'timestamp': self.timestamp, 'value': CollectionTime_rate, 'check_type': check_type, 'chart_type': 'Rate'})
                        if ky is 1:
                            value = j['value'][vl]
                            v = check_null(value)
                            self.local_vars.append({'name': 'spark_master_g1' + type + vl.lower(), 'timestamp': self.timestamp, 'value': v, 'check_type': check_type, 'reaction': reaction})
