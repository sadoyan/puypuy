...
def runcheck():
    ...
    try:
        ...
        if CMS is True:
            ...
            for coltype in collector:
                ...
                def push_metrics(preffix):
                    CollectionTime_rate = rate.record_value_rate('activemq_' + preffix + '_CollectionTime', CollectionTime, timestamp)
                    local_vars.append({'name': 'activemq_' + preffix + '_lastgcinfo', 'timestamp': timestamp, 'value': LastGcInfo, 'check_type': check_type})
                    local_vars.append({'name': 'activemq_' + preffix + '_collection_count', 'timestamp': timestamp, 'value': CollectionCount, 'check_type': check_type})
                    local_vars.append({'name': 'activemq_' + preffix + '_collectiontime', 'timestamp': timestamp, 'value': CollectionTime_rate, 'check_type': check_type, 'chart_type': 'Rate'})
            ...
            for k, v in enumerate(gc_g1):
                ...
                for ky, vl in enumerate(metr_keys):
                    if ky is 0:
                        ...
                        v = check_null(value)
                        rate_key = vl + type
                        CollectionTime_rate = rate.record_value_rate('activemq_' + rate_key, v, timestamp)
                        local_vars.append({'name': 'activemq_g1' + type + vl.lower(), 'timestamp': timestamp, 'value': CollectionTime_rate, 'check_type': check_type, 'chart_type': 'Rate'})
                    if ky is 1:
                        value = j['value'][vl]
                        v = check_null(value)
                        local_vars.append({'name': 'activemq_g1' + type + vl.lower(), 'timestamp': timestamp, 'value': v, 'check_type': check_type, 'reaction': reaction})
        ...
        for s in bstats:
            local_vars.append({'name': 'activemq_' + s.lower(), 'timestamp': timestamp, 'value': brokerstats['value'][s], 'check_type': check_type})

        #lib.puylogger.print_message(str(brokerstats['value']))
