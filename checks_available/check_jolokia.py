        ...
        if CMS is True:
            ...
            for coltype in collector:
                ...
                def push_metrics(preffix):
                    ...
                    local_vars.append({'name': 'jolokia_'+preffix+'_lastgcinfo', 'timestamp': timestamp, 'value': LastGcInfo, 'check_type': check_type})
                    local_vars.append({'name': 'jolokia_' + preffix + '_collection_count', 'timestamp': timestamp, 'value': CollectionCount, 'check_type': check_type})
                    local_vars.append({'name': 'jolokia_' + preffix + '_CollectionTime', 'timestamp': timestamp, 'value': CollectionTime_rate, 'check_type': check_type, 'chart_type': 'Rate'})
            ...
            for k, v in enumerate(gc_g1):
                ...
                for ky, vl in enumerate(metr_keys):
                    if ky is 0:
                        ...
                        local_vars.append({'name': 'jolokia_g1' + type + vl.lower(), 'timestamp': timestamp, 'value': CollectionTime_rate, 'check_type': check_type, 'chart_type': 'Rate'})
                    if ky is 1:
                        value = j['value'][vl]
                        v = check_null(value)
                        local_vars.append({'name': 'jolokia_g1' + type + vl.lower(), 'timestamp': timestamp, 'value': v, 'check_type': check_type, 'reaction': reaction})
