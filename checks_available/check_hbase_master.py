                ...
                for values in node_stuck_keys:
                    if values in stats_keys[stats_index]:
                        if values == 'HeapMemoryUsage':
                            ...
                            self.local_vars.append({'name': 'hmaster_node_' + values.lower(), 'timestamp': self.timestamp, 'value': stats_keys[stats_index][values], 'check_type': check_type, 'chart_type': 'Rate'})
